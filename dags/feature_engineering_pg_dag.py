...
RAW_PATH = Variable.get("raw_path", f"s3://{S3_BUCKET}/raw/target.xlsx")
FEATURES_PATH = Variable.get("features_path", f"s3://{S3_BUCKET}/features/df_with_features_1.csv")


@dag(
    dag_id="feature_engineering",
    description="Расчёт признаков с сохранением в S3, по умолчанию признаки сохраняются в features/df_with_features_1.csv",
    catchup=False,
    tags=["ml", "features", "s3"],
)
